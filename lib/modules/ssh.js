...
class Ssh {
   ...
   static async _JSSSHExec(cmd, sshConfig, timeout = 5000, verbose = false, options = { }) {
        let buffer = '';
        let closing = false;
        ...
        return new Promise((resolve, reject) => {
            ...
            .on('ready', () => {
                    ...
                    if (err.message.indexOf('ECONNRESET') >= 0 || err.message.indexOf('ECONNREFUSED') >= 0 || err.message.indexOf('Timed out while waiting for handshake') >= 0) {
                        ...
                        console.error(`Waiting 5 second for ${sshConfig.hostname}:${sshConfig.port} to be ready`);
                        setTimeout(async () => {
                            resolve(await Ssh._JSSSHExec(cmd, sshConfig, timeout, verbose, options));
                        }, 5000);
                    } else {
                        reject(err);
                    }
