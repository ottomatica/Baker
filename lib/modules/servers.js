    ...
    static async installBakerServer(forceVirtualBox = false) {
        if (require('os').platform() === 'darwin' && !forceVirtualBox) {
            ...
            if ((await provider.driver.list()).filter(e => e.name === 'baker-srv').length == 0) {
                let isoURL = 'https://github.com/ottomatica/baker-release/releases/download/latest-dev/alpine.iso';
                let isoPath = path.join(boxes, 'boxes', 'alpine.iso');
                if (!(await fs.existsSync(isoPath)) || (await md5File(isoPath)) != '851e2b2b34e31b67aa0758d25666e8e5') {
                    await download(isoURL, path.join(boxes, 'boxes'));
                }
