...
class VaultLib {
    ...
    vault(filePath, passphrase)
    {
        ...
        let buffer = `baker-vault:${version}:${this.iv.toString('hex')}:${shasum.digest('hex')}\n`;
    ...
    retrieve(filePath, passphrase)
    {
        if( this.isEncrypted(filePath) )
        {
            ...
            let sha = lines[0].split(':')[3];
            let recovered = this.decryptWithKey(lines[1],key,iv);

            let shasum = crypto.createHash('sha512');
            shasum.update(recovered);

            if( sha == shasum.digest('hex') )
            {
                return recovered;
            }
            throw new Error("Incorrect vault password supplied.")
        }
