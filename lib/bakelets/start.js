...
async function start(cmd, vmName, ansibleSSHConfig, verbose) {
    ...
    await Ssh.sshExec(`export ANSIBLE_HOST_KEY_CHECKING=false && cd /home/vagrant/baker/${vmName} && ansible all -m shell -a 'nohup bash -c "${cmd}" > ~/start.out 2> ~/start.err &' -i baker_inventory -v`, ansibleSSHConfig, 20000, verbose);
}
